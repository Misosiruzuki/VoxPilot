...
AGENT_JAR="$(ls -1 "$AGENT_DIR"/build/libs/voxpilot-agent-*.jar | grep -v 'dev\|sources\|javadoc' | head -1)"
...
# examples optional inside jar for convenience
if [[ -d examples ]]; then
